...
StartEvent.OnServerEvent:Connect(function(player)
    if DEBUG then print("[TempestKick] StartEvent from", player.Name) end
    local char = player.Character
    local humanoid = char and char:FindFirstChildOfClass("Humanoid")
    if not humanoid then
        if DEBUG then print("[TempestKick] No humanoid") end
        return
    end
    if StunService:IsStunned(player) or StunService:IsAttackerLocked(player) then
        if DEBUG then print("[TempestKick] Player stunned or locked") end
        return
    end
    local tool = char:FindFirstChildOfClass("Tool")
    if not tool or tool.Name ~= "Rokushiki" then
        if DEBUG then print("[TempestKick] Invalid tool") end
        return
    end
    if not StaminaService.Consume(player, 75) then
        if DEBUG then print("[TempestKick] Not enough stamina") end
        return
    end
...
HitEvent.OnServerEvent:Connect(function(player, targets, dir)
    if DEBUG then print("[TempestKick] HitEvent from", player.Name) end
    if typeof(targets) ~= "table" then
        if DEBUG then print("[TempestKick] Invalid targets") end
        return
    end
    ...
    if not humanoid or not hrp then
        if DEBUG then print("[TempestKick] Missing humanoid or HRP") end
        return
    end

    local tool = char:FindFirstChildOfClass("Tool")
    if not tool or tool.Name ~= "Rokushiki" then
        if DEBUG then print("[TempestKick] Invalid tool during hit") end
        return
    end
    ...
    for _, enemyPlayer in ipairs(targets) do
        ...
        if not StunService:CanBeHitBy(player, enemyPlayer) then
            if DEBUG then print("[TempestKick] Cannot hit", enemyPlayer.Name) end
            continue
        end
        ...
        elseif blockResult == "Damaged" then
            if DEBUG then print("[TempestKick] Block damaged", enemyPlayer.Name) end
            continue
        elseif blockResult == "Broken" then
            if DEBUG then print("[TempestKick] Block broken", enemyPlayer.Name) end
        ...
        local dmg = TempestKickConfig.Damage
        ...
        StunService:ApplyStun(enemyHumanoid, TempestKickConfig.StunDuration, false, player, true)
